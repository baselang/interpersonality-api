...
"""API Module to Update Terms and Condition related permissions of the user on Privacy page.
...
- Update the terms and condition related permissions of the user.
...
config.read('update_consent.properties', encoding = "ISO-8859-1")
...
def handler(event,context):
    """Function to handle the request for Update Consent API"""
    logger.info(event)
    ...
    try:
        ...
        # fetching the variables from data
        is_customized = None if data['is_customized'] is None else int(data['is_customized'])
        is_email = None if data['is_email'] is None else int(data['is_email'])
        is_ads = None if data['is_ads'] is None else int(data['is_ads'])
        is_privacy_consent = int(data['is_privacy_consent'])
    except:
    ...
    try:
        ...
        if is_customized != None and is_customized != 1:
            # if is_customized tag is 
            logger.error("is_customized : " + str(is_customized))
    ...
    try:
        ...
        try:
            # Constructing the query to update the permissions related to the user
            if is_customized != None and is_email != None and is_ads != None:
                
                query = "UPDATE `user_permissions` SET `is_customized`=%s,`is_email`=%s, `is_ads`=%s, `is_privacy_consent`=%s WHERE `rid`=%s"
                # Executing the query using cursor
                cursor.execute(query, (is_customized, is_email, is_ads, is_privacy_consent, rid))
            else:
                query = "UPDATE `user_permissions` SET `is_privacy_consent`=%s WHERE `rid`=%s"
                # Executing the query using cursor
                cursor.execute(query, (is_privacy_consent, rid))
